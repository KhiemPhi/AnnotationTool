...
def _create_text_labels(classes, scores, class_names, score_show):
    """
    Args:
...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions, score_show):
        """
        ...
        """
        boxes = np.array(predictions["pred_boxes"])       
        ...
        if self._instance_mode == ColorMode.IMAGE_BW:
            self.output.img = self._create_grayscale_image(
                (predictions.pred_masks.any(dim=0)
                 > 0).numpy()
            )
            alpha = 0.3
